...
import json
import torch
from model.attn_processor import AttnProcessor2_0, SkipAttnProcessor,CatAttnProcessor
#from ip_adapter.attention_processor import IPAttnProcessor
from diffusers.models.attention_processor import IPAdapterAttnProcessor  # Diffusers 官方实现
    ...
    for name in unet.attn_processors.keys():
        ...
        if cross_attention_dim is None:
            if self_attn_cls is not None:
                attn_procs[name] = self_attn_cls(hidden_size=hidden_size, cross_attention_dim=cross_attention_dim,
                                                 **kwargs)
            else:
            ...
            attn_procs[name] = cross_attn_cls(hidden_size=hidden_size, cross_attention_dim=cross_attention_dim,
                                              **kwargs)
...
# def init_adapter(unet,
#                  cross_attn_cls=SkipAttnProcessor,
#                  self_attn_cls=CatAttnProcessor,
#                  cross_attn_dim=None,
#                  image_encoder=None,
#                  **kwargs):
#     if cross_attn_dim is None:
#         cross_attn_dim = unet.config.cross_attention_dim
#     attn_procs = {}
#     features_image=image_encoder(image)
#     for name in unet.attn_processors.keys():
#         cross_attention_dim = None if name.endswith("attn1.processor") else cross_attn_dim
#         if name.startswith("mid_block"):
#             hidden_size = unet.config.block_out_channels[-1]
...
#             block_id = int(name[len("down_blocks.")])
#             hidden_size = unet.config.block_out_channels[block_id]
#         if cross_attention_dim is None:  # 自注意层
#             if self_attn_cls is not None:
#                 # 检查维度并创建投影层
#                 k_proj, v_proj = None, None
#                 if features_image.size(-1) != hidden_size:
#                     k_proj = nn.Linear(features_image.size(-1), hidden_size)
#                     current_k_tokens = k_proj(current_k_tokens)
#                 if features_image.size(-1) != hidden_size:
#                     v_proj = nn.Linear(features_image.size(-1), hidden_size)
#                     current_v_tokens = v_proj(features_image)
#                 # 初始化自注意力处理器
#                 processor = self_attn_cls(
#                     hidden_size=hidden_size,
#                     cross_attention_dim=cross_attention_dim,
#                     k_tokens=current_k_tokens,
#                     v_tokens=current_v_tokens,
#                     **kwargs
#                 )
#                 # 将投影层添加到处理器子模块中
#                 if k_proj is not None:
#                     processor.add_module('k_proj', k_proj)
#                 if v_proj is not None:
#                     processor.add_module('v_proj', v_proj)
