...
def parse_args():
    ...
    parser.add_argument("--output_dir", type=str,
                        default="/mnt/sda2/lry/ckpt_dream_edit-midBlock_noLoss")
...
def train(args):
    ...
    for name in unet.attn_processors.keys():
        ...
        if cross_attention_dim is None:
            ...
            #                                     )  # 跳过自注意力
            # midblock层自注意力替换
            if name.startswith("mid_block"):# or name.startswith("up_blocks.1"): # or name.startswith("down_blocks.2"):
                attn_procs[name] = SkipAttnProcessor(hidden_size=hidden_size, cross_attention_dim=cross_attention_dim)
    ...
    for name, param in unet.named_parameters():
        if 'attn1' in name and 'up_blocks' in name and ('to_q' in name or 'to_k' in name or 'to_v' in name or 'to_out' in name):  # or 'to_out' in name
        #if 'attn1' in name and ('to_out' in name):
            param.requires_grad = True
        if 'attn1' in name and 'down_blocks' in name and ('to_q' in name or 'to_k' in name or 'to_v' in name or 'to_out' in name):  #  or 'to_out' in name
        #if 'attn1' in name and ('to_out' in name):
            param.requires_grad = True
        if 'attn2' in name and 'mid_block' in name and ('to_q' in name or 'to_out' in name):  #  or 'to_out' in name
        #if 'attn1' in name and ('to_out' in name):
            param.requires_grad = True
        # if 'attn1' in name and 'up_blocks' in name and (
    ...
    ip_adapter = IPAdapter(unet, image_proj_model,  None)
    weight_dtype = torch.float32
    ...
    params_to_opt = itertools.chain(ip_adapter.image_proj_model.parameters(), unet.parameters())
    # 打印所有训练参数的名称和形状
    print("===== 训练参数列表 =====")
    total_params = 0
    ...
    if checkpoint_dirs:
        ...
        accelerator.load_state(latest_checkpoint, strict=True)
        #ip_adapter.adapter_modules.load_state_dict(latest_checkpoint, strict=False)
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(ip_adapter):
                ...
                with torch.no_grad():
                    ...
                    image_embeds_ = []
                    for image_embed,cloth_latents_embed, drop_image_embed in zip(image_embeds,cloth_latents, batch["drop_image_embeds"]):
            ...
            if accelerator.is_main_process:
                ...
                if  global_step == 5 or global_step == 100 or global_step == 1000 or global_step == 3000 or global_step == 5000 or global_step == 8000:
                    print("Saving model...")
                    logging.info(
                        f"Epoch {epoch}, Step {global_step}: Loss {loss.item()}, LR {lr_scheduler.get_last_lr()[0]}"
                    )
