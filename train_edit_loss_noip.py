...
def train(args):
    ...
    # 使用 safetensors 加载
    try:
        unet_state_dict = load_file(args.unet_ckpt)
    ...
    #unet.load_state_dict(unet_dict, strict=True)  # 根据实际情况调整 strict
    # initialized to zero.
    ...
    for epoch in range(args.num_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                # 将 timesteps 改为全 0
               # timesteps = torch.zeros((bsz,), dtype=torch.long, device=noise.device)  # 直接生成全0张量
                timesteps = timesteps.long()  # 可省略（因为 dtype 已指定为 long）
                # 强制10%样本使用t=0（根据实验调整比例）
                force_t1_mask = torch.rand(timesteps.shape, device=timesteps.device) < 0.05
            ...
            if accelerator.is_main_process:
                ...
                if  global_step==200 or global_step==400 or global_step==1000 or global_step==21000 or global_step==22000:
                    print("Saving model...")
                    logging.info(
                        f"Epoch {epoch}, Step {global_step}: Loss {loss.item()}, LR {lr_scheduler.get_last_lr()[0]}"
                    )
...
def parse_args():
    ...
    parser.add_argument("--unet_ckpt", type=str,
                       default='/mnt/sda2/lry/catViton_modelckpt_noip_384-512_dim-2_dream_######_out/checkpoint-20000/model.safetensors')
    ...
    parser.add_argument("--output_dir", type=str, default="/mnt/sda2/lry/catViton_modelckpt_ramdom_step_noip_0.05loss_dream_######_out")
